...
public class LogicLoom implements ModInitializer
{
    ...
    public void onInitialize()
    {
        CommandRegistrationCallback.EVENT.register((dispatcher, registryAccess, environment) -> {
            dispatcher.register(CommandManager.literal("logicloom").requires(CommandManager.requirePermissionLevel(CommandManager.GAMEMASTERS_CHECK))
                    ...
                    .then(CommandManager.literal("start_pos").executes(LogicLoom::PrintStartPos).then(CommandManager.argument("block_pos", BlockPosArgumentType.blockPos()).executes(LogicLoom::SetBlockPos)))
                    ...
                    .then(CommandManager.literal("param")
                            .then(CommandManager.literal("max_iter")
                                            .then(CommandManager.argument("max_iter", IntegerArgumentType.integer(0, 10_000)).executes(LogicLoom::SetMaxIter))
                            .executes(LogicLoom::GetMaxIter)
                            )
                            .then(CommandManager.literal("router_max_search")
                                    .then(CommandManager.argument("router_max_search", IntegerArgumentType.integer(0)).executes(LogicLoom::SetMaxSearch))
                                    .executes(LogicLoom::GetMaxSearch)
                            )
                            .then(CommandManager.literal("force_const")
                                            .then(CommandManager.argument("force_const", DoubleArgumentType.doubleArg()).executes(LogicLoom::SetForceConst))
                            .executes(LogicLoom::GetForceConst)
                            )
                            .then(CommandManager.literal("force_mul")
                                    .then(CommandManager.argument("force_mul", DoubleArgumentType.doubleArg()).executes(LogicLoom::SetForceMul))
                                    .executes(LogicLoom::GetForceMul)
                            )
                            .then(CommandManager.literal("chip_size")
                                    .then(CommandManager.argument("chip_size", IntegerArgumentType.integer(0, 10_000)).executes(LogicLoom::SetChipSize))
                            .executes(LogicLoom::GetChipSize)
                            )
                            .then(CommandManager.literal("do_actual_place")
                                    .then(CommandManager.argument("do_actual_place", BoolArgumentType.bool()).executes(LogicLoom::SetActualPlace))
                            .executes(LogicLoom::GetActualPlace)
                            )
                            .then(CommandManager.literal("do_legalization")
                                    .then(CommandManager.argument("do_legalization", BoolArgumentType.bool()).executes(LogicLoom::SetLegalization))
                    ...
                    .then(CommandManager.literal("misc")
                            ...
                            .then(CommandManager.literal("check_wires").executes(LogicLoom::CheckWires))
                            .then(CommandManager.literal("show_boundary").executes(LogicLoom::ShowBoundBox))
                            .then(CommandManager.literal("wipe").executes(LogicLoom::Wipe))
                    )
                    ...
                    .then(CommandManager.literal("debug")
                            ...
                            .then(CommandManager.literal("vcddebug").then(CommandManager.argument("file_path", StringArgumentType.greedyString()).executes(LogicLoom::VCDDebug)))
                            .then(CommandManager.literal("vcdcomp").executes(LogicLoom::VCDComp))
                            .then(CommandManager.literal("test_cell_port").then(CommandManager.argument("cell_type", StringArgumentType.word()).suggests(CellTypeSuggestionProvider.Provider()).then(CommandManager.argument("pos", BlockPosArgumentType.blockPos()).executes(LogicLoom::TestCellPorts))))
        ...
        ResourceLoader.get(ResourceType.SERVER_DATA).registerReloader(
            Identifier.of("logicloom", "logic_loom_res"),
            (SynchronousResourceReloader) manager -> {
                var dat2 = manager.getResource(Identifier.of("logicloom:flute_data/post9.dat"));
                var dat1 = manager.getResource(Identifier.of("logicloom:flute_data/powv9.dat"));
    ...
    public static int CheckWires(CommandContext<ServerCommandSource> context)
    {
        ...
        {
            context.getSource().sendError(Text.literal("You must place wires using /logicloom route before using this command"));
            return 0;
        }
        if (Misc.CheckStartPos(context))
        {
